...
void loop() {
    ...
    if(Serial.available()){                          //Send commands over serial to play
      switch(Serial.read()){
        case 'r': audio.startRecording("test.wav",20000,A0); break;    //Record at 16khz sample rate on pin A0
        case 'R': audio.startRecording("test.wav",20000,A0,1); break;  //Record, but with passthrough to speaker.
        case 't': audio.startRecording("test.wav",20000,A0,2); break;  //Do not record. Output direct to speaker
